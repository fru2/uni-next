...
export const GET = async (req) => {
  try {
    ...
    const authorConditions = [
      ...
      { 'Authors.a3': { $exists: true } }, // All other authors must not exist
    ...
    const threeAuthorPapers = await ResearchPaper.find({
      $and: authorConditions,
    });

    return new NextResponse(JSON.stringify({ result: threeAuthorPapers }), {
      status: 200,
    });
  } catch (error) {
